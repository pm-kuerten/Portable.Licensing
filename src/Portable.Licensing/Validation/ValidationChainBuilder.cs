    ...
    internal class ValidationChainBuilder : IStartValidationChain, IValidationChain
    {
        ...
        public IEnumerable<IValidationFailure> AssertValidLicense()
        {
            ...
            while (_validators.Count > 0)
            {
                ...
                if (validator.ValidateWhen is not null && !validator.ValidateWhen(_license))
                    continue;
                if (validator.Validate is null) throw new Exception("No validation function set");
                if (!validator.Validate(_license)) yield return validator.FailureResult ?? new GeneralValidationFailure("License validation failed!");
            }
        }
        #endregion
    }
